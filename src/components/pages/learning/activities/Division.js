...
// Dynamic problem generator for division (1, 2, and 3 digits)
const generateDivisionProblem = () => {
  ...
  if (random < 0.4) {
    // 1-digit numbers (1-9) - ensure clean division
    op2 = Math.floor(Math.random() * 9) + 1;
    const result = Math.floor(Math.random() * 9) + 1;
    op1 = op2 * result;
  } else if (random < 0.8) {
    // 2-digit numbers (10-99) - ensure clean division
    op2 = Math.floor(Math.random() * 20) + 10; // Divisor 10-29
    const result = Math.floor(Math.random() * 9) + 1;
    op1 = op2 * result;
  } else {
    // 3-digit numbers (100-999) - ensure clean division
    op2 = Math.floor(Math.random() * 50) + 50; // Divisor 50-99
    const result = Math.floor(Math.random() * 9) + 1;
    op1 = op2 * result;
  }
        ...
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="mt-8 text-center pb-6">
            <p className="text-gray-600">
              Solve the division problem and click Next for a new one!
            </p>
          </div>
        </div>
      </div>
