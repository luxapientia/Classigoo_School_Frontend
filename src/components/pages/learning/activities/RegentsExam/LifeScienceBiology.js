...
export default function LifeScienceBiology({ user, grade }) {
  ...
  const fetchRandomQuestion = async () => {
    ...
    try {
      ...
      const { data: res } = await axios.get(`/v1/learning/nys/biology/random?grade=${gradeNumber}`);
  ...
  const fetchExamQuestions = async (questionCount) => {
    ...
    try {
      const gradeNumber = Math.max(Number(grade), 3);
      const { data: res } = await axios.get(`/v1/learning/nys/biology/exam?grade=${gradeNumber}&count=${questionCount}`);
      if (res.status === "success" && res.data) {
  ...
  if (mode === "exam" && examQuestions.length > 0 && !examCompleted) {
    ...
    return (
      <RegentsExamActivityLayout title="Life Science Biology - Exam" currentIndex={0} backgroundIndex={0} grade={grade}>
        {imageVisible && (
  ...
  if (mode === "practice") {
    if (loading) {
      return (
        <RegentsExamActivityLayout title="Life Science Biology" currentIndex={0} backgroundIndex={0} grade={grade}>
    ...
    return (
      <RegentsExamActivityLayout title="Life Science Biology - Practice" currentIndex={0} backgroundIndex={0} grade={grade}>
        {imageVisible && (
          <div className="flex w-full justify-center items-center absolute left-0 right-0 top-0 z-20">
            <Image
              src={`/assets/img/screen/${imageName}.png`}
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
          <h1 className="text-2xl md:text-3xl font-semibold text-white text-center mb-8 mt-4 drop-shadow">
            Practice Mode - Answer the Life Science Biology question below.
          </h1>
          <div className="bg-white rounded-xl shadow-lg px-6 py-8 max-w-2xl w-full flex flex-col items-center">
            <div className="mb-6 text-center">
              <span className="text-lg md:text-xl text-gray-900 font-medium">
  ...
  return (
    <RegentsExamActivityLayout title="Life Science Biology" currentIndex={0} backgroundIndex={0} grade={grade}>
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <h1 className="text-2xl md:text-3xl font-semibold text-white text-center mb-8 mt-4 drop-shadow">
          Welcome to Life Science Biology
        </h1>
        <div className="bg-white rounded-xl shadow-lg px-8 py-6 max-w-md w-full text-center">
          <p className="text-gray-600 mb-4">Please select your preferred mode to continue.</p>
          <button
