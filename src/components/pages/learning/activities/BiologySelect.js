...
export default function BiologySelect({ user, grade }) {
  ...
  const fetchRandomQuestion = async () => {
    ...
    try {
      ...
      const { data: res } = await axios.get(`/v1/learning/biology/random?grade=${gradeNumber}`);
      ...
      console.error("Error fetching biology question:", err);
      setQuestion(null);
    }
    setLoading(false);
  };
  ...
  return (
    <LearningScreenActivityLayout title="Biology" currentIndex={3} backgroundIndex={3} grade={grade}>
      {imageVisible && (
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <h1 className="text-2xl md:text-3xl font-semibold text-white-700 text-center mb-8 mt-4 drop-shadow">
          Solve the biology problem below.
        </h1>
