...
export default function ChemistrySelect({ user, grade }) {
  ...
  const fetchRandomQuestion = async () => {
    ...
    try {
      ...
      const { data: res } = await axios.get(`/v1/learning/chemistry/random?grade=${gradeNumber}`);
      ...
      console.error("Error fetching chemistry question:", err);
      setQuestion(null);
    }
    setLoading(false);
  };
  ...
  return (
    <LearningScreenActivityLayout title="Chemistry" currentIndex={4} backgroundIndex={4} grade={grade}>
      {imageVisible && (
