...
export default function MathsSelect({ user, grade }) {
  ...
  const fetchRandomQuestion = async () => {
    ...
    try {
      ...
      const { data: res } = await axios.get(`/v1/learning/maths/random?grade=${gradeNumber}`);
      ...
      console.error("Error fetching maths question:", err);
      setQuestion(null);
    }
    setLoading(false);
  };
  ...
  return (
    <LearningScreenActivityLayout title="Maths" currentIndex={1} backgroundIndex={1} grade={grade}>
      {imageVisible && (
